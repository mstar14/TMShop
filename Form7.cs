...
namespace _02
{
    public partial class Form7 : Form
    {
        ...
        private void ButtonAdd_Click(object sender, EventArgs e)
        {
            ...
            int sum = 0;
            if (cmbGender.Text == "Tuxedo")
            {
                int p = int.Parse(cmbNumber.Text);
                sum = sum + (p * 3500);
                C1.Price = sum;
            }
            else if (cmbGender.Text == "Suit")
            {
                int p = int.Parse(cmbNumber.Text);
                sum = sum + (p * 5500);
                C1.Price = sum;
            }
            else if (cmbGender.Text == "Vest")
            {
                int p = int.Parse(cmbNumber.Text);
                sum = sum + (p * 2000);
                C1.Price = sum;
        ...
        {
            MessageBox.Show("กำการซื้อสำเร็จ กรุณารอรับสินค้า");
            this.Hide();
        ...
        private void DgvContactList_RowHeaderMouseClick_1(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            cmbGender.Text = dgvContactList.Rows[rowsIndex].Cells[6].Value.ToString();
            cmbNumber.Text = dgvContactList.Rows[rowsIndex].Cells[7].Value.ToString();
            cmbSize.Text = dgvContactList.Rows[rowsIndex].Cells[5].Value.ToString();
